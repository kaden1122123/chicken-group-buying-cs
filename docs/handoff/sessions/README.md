...
| Session | 業務問題（CEO 視角）| 推薦 | 估時 |
|---------|---------------------|------|------|
| **E** | 客戶按「確認訂購」按鈕沒反應，訂單沒成立 | 🔴 高 | 1-2 小時決策 + 實作 |
| **D3** | 改 chicken.yaml 的業務規則沒效果（要工程師改 code）| 🔴 高 | 2-3 小時 |
| **D4** | 9 個開關永遠當啟用（config 寫 false 沒用）| 🔴 高 | 2 小時 |
| **F** | 文件寫的跟實際對不上（測試套數等）| 🟡 中 | 1.5 小時 |
| **G** | 沒有自動化測試、沒有 ESLint | 🟡 中 | 2-3 小時 |
| **H** | 6 個重要模組沒有 unit test | 🟡 中 | 3-4 小時 |
| **J** | sync-mirror 會誤刪主位置測試資料 | 🟢 低 | 1-2 小時 |
| **K** | log 格式不一致，難以除錯 | 🟢 低 | 2 小時 |
