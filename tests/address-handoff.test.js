...
/**
 * P0-1: addressRule 觸發 handoff 驗證測試（node:test 風格 · P1-4）
 *
...
test('addressRule 對超出配送範圍回 handoff_needed', () => {
...
test('addressRule 對需人工確認回 handoff_needed', () => {
  ...
  assert.ok(r.errorMessage.includes('需由客服進一步確認'));
});

test('addressRule 對空地址回 reask', () => {
  const r = validateAddress('');
  assert.strictEqual(r.valid, false);
  assert.strictEqual(r.action, 'reask');
  assert.ok(r.errorMessage.includes('地址為必填'));
});

test('addressRule 對合法地址回 valid:true 無 action', () => {
  const r = validateAddress('三峽北大特區學成路100號');
...
test('awaitingInfo 對超出範圍傳上 handoff_needed', () => {
...
test('awaitingInfo 對需人工確認傳上 handoff_needed', () => {
...
test('awaitingInfo 對空地址仍走 validation_failed', () => {
...
// ═════════════════════════════════════════════════════════════════
// 3. handleMessage 對 handoff_needed 觸發 handleHandoff
// ═════════════════════════════════════════════════════════════════

test('handleMessage 大溪區地址觸發 handoff，新狀態 = HUMAN_HANDOFF', async () => {
...
test('handleMessage 台北市信義區也轉 HUMAN_HANDOFF', async () => {
...
test('handleMessage 合法地址不觸發 handoff', async () => {
...
test('out_of_range reason 寫入 staff_notes', async () => {
...
test('needs_confirmation reason 寫入 staff_notes', async () => {
...
test('一般 handoff（退款）staff_notes 不含 address reason', async () => {
