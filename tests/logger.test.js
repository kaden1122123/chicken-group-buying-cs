...
test('1. parseLevel 正確處理字串、數字、unknown', () => {
  const l = loadLogger();
  try {
...
test('3. getThreshold 從 env 動態讀取 LOG_LEVEL', () => {
  let l = loadLogger();
  try {
...
test('4. JSON 輸出格式（timestamp + level + msg + meta）', () => {
...
test('5. stream routing — debug/info → stdout, warn/error → stderr', () => {
  const l = loadLogger('debug');
  try {
...
test('6. level threshold filter — LOG_LEVEL=warn 時 debug/info 被過濾', () => {
  ...
  try {
    ...
    assert.ok(captured.stderr.includes('warn msg'));
    assert.ok(captured.stderr.includes('error msg'));
    assert.ok(!captured.stderr.includes('debug msg'));
  } finally { l.restore(); }
});

test('7. level threshold filter — LOG_LEVEL=error 時 warn 也被過濾', () => {
  const l2 = loadLogger('error');
  try {
    const c2 = captureStreams(() => {
      l2.logger.warn('warn-filtered');
      l2.logger.error('error-shown');
    });
    assert.ok(!c2.stderr.includes('warn-filtered'));
    assert.ok(c2.stderr.includes('error-shown'));
  } finally { l2.restore(); }
});

test('8. meta handling — object / 不覆蓋 / 非 object / undefined', () => {
  const l = loadLogger('debug');
  try {
    // object meta → spread
...
test('9. msg type coercion — number / null / object', () => {
  const l = loadLogger('debug');
  try {
    ...
    entry = parseLastJsonLine(c.stdout);
    assert.strictEqual(entry.msg, '', 'null → 空字串');

    c = captureStreams(() => l.logger.info({ foo: 'bar' }));
    entry = parseLastJsonLine(c.stdout);
    assert.strictEqual(entry.msg, '[object Object]', 'object → toString');
  } finally { l.restore(); }
});

test('10. Error.message 作為 meta 欄位', () => {
  const l = loadLogger('debug');
  try {
    const c = captureStreams(() => l.logger.error('operation failed', { err: 'test error' }));
...
test('11. LOG_LEVEL=debug 時全部輸出', () => {
  ...
  try {
    ...
    assert.ok(c3.stdout.includes('debug-shown'));
    assert.ok(c3.stdout.includes('info-shown'));
  } finally { l3.restore(); }
});
